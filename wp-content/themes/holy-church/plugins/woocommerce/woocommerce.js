/* global jQuery, HOLY_CHURCH_STORAGE */

jQuery( document ).on( 'action.ready_holy_church', function() {
	...
	$document.on( 'action.resize_holy_church', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.holy_church_shop_mode_buttons a', function(e) {
		...
		holy_church_set_cookie( 'holy_church_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	holy_church_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function holy_church_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						holy_church_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function holy_church_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function(response) {
						...
						holy_church_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						holy_church_import_tags_link( response );
	...
	$document.on( 'action.scroll_holy_church', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < holy_church_window_scroll_top() + holy_church_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		holy_church_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					holy_church_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					holy_church_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		holy_church_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			holy_church_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function holy_church_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = holy_church_window_width() + ( holy_church_window_height() < $document.height() || holy_church_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = table_cart;

				if ( w < HOLY_CHURCH_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= HOLY_CHURCH_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
